...
- **Green avatar + name** = an online player, at their real in-game position.
...
- **"Not connected" badge won't clear**: check that the dashboard's PC can still reach the Rust server's RCON port - nothing else to do on the dashboard side, it retries automatically.
