...
- **Add / Remove From Group**: pick a group from the dropdown (populated from your server's actual groups, not typed blind) and add or remove a player. Below it, **Create New Group** makes a brand new one (name required, title optional) - it shows up in the dropdown right after, though occasionally you may need to switch tabs and back if the list doesn't refresh immediately (the server's response to a group list lookup is occasionally flaky right after a change - the group itself is created either way, this is just the dropdown catching up).
