...
case $choice in
        ...
        while true; do
            ...
            case $subchoice in
                ...
                5) read -p "Are you sure you want to start the Rust server? (Y/n): " confirm_start
                    ...
                    ;;
                0) echo "Going Back " && clear && $AMAP && break ;;
    ...
    2)
        while true; do
            ...
            echo "2. Tail Logs"
            echo "3. Clear Logs"
            echo "4. Tail Rust Server Console"
            echo "0. Back to Menu"
            read -p "Enter your choice: " subchoice
            case $subchoice in
                ...
                3) read -p "Are you sure you want to CLEAR SERVER LOGS? (Y/n): " confirm_LogCleaner
                    ...
                    ;;
                4) trap "$AMAP" EXIT
                        tail -f /home/$USERNAME/log/console/rustserver-console.log ;;
                0) echo "Going Back " && sleep 2 && clear && $AMAP && exit ;;
                *) echo "Invalid choice. Please enter a valid sub-option." ;;
            esac
            read -p "Press Enter to continue..."
        done
                ...
                3)
                    while true; do
                        ...
                        case $subchoice in
                            ...
                            0) echo "Going Back " && clear && $AMAP && exit ;;
                            *) echo "Invalid choice. Please enter a valid sub-option." ;;
                        esac
                        read -p "Press Enter to continue..."
                    done
            ...
            case $subchoice in
                ...
                2) read -p "Are you sure you want to CLEAR WIPE CONFIGURATIONS? (Y/n): " confirm_WipeClear
                    if [ "$confirm_WipeClear" == "Y" ]; then
                        echo > $DIR/Files/Config/config.cfg && echo > $DIR/Files/Config/rustserver.cfg
                    else
                        echo "Clear Wipeconfiguration Aborted." && $AMAP && exit
                    fi
                    ;;
                3) cat $DIR/Files/Config/config.cfg ;;
                0) echo "Going Back " && clear && $AMAP && exit ;;
                *) echo "Invalid choice. Please enter a valid sub-option." ;;
            esac
            read -p "Press Enter to continue..."
...
7) while true; do
    ...
    cat $DIR/$RUSTINSTALL
    echo " Choose an option"
    echo "1. Backup Folder Size"
    echo "2. Server Online/Ofline Status"
    echo "3. Map Seed"
    echo "4. Map Size"
    echo "5. Player Count"
    echo "6. BTOP"
    ...
    read -p "Enter your choice: " subchoice
    case $subchoice in
        1) du -sh /home/$USERNAME/BackUps/ ;;
        2) cat $DIR/Files/Images/Status ;;
        3) cat /home/$USERNAME/LIVE_FILES/ServerStatus |grep seed ;;
        4) cat /home/$USERNAME/LIVE_FILES/ServerStatus |grep size ;;
        5) cat /home/$USERNAME/LIVE_FILES/ServerStatus |grep players ;;
        6) btop ;;
        0) echo "Going Back " && clear && $AMAP && exit ;;
        *) echo "Invalid choice. Please enter a valid sub-option." ;;
    esac
...
;;
        ...
        8) cat $DIR/ReadMe.md && $AMAPNC && exit ;;
        9) clear && $AMAP ;;
        10) $UPDATER && $AMAP && exit ;;
        0) echo "Exiting AMAP." && sleep 1 && clear && exit ;;
        *) echo "Invalid choice. Please enter a number between 1 and 11." && sleep 1 && clear && $AMAP ;;
    esac
