...
- **Players** - online players (name, SteamID, IP, ping, session/total time, last connected, Rust hours) with one-click kick, ban/unban (reason required and logged), and Look up; plus recently-seen offline players (with their last-known IP), currently-banned players, and a per-player notes log (kick and ban reasons are both added automatically), with a search box to find a keyword across every player's notes at once instead of needing a SteamID first. Notes and stats (total time, last connected, last IP) are shared with any other admin running their own copy of this dashboard, if Settings > Plugin Deploy is configured - see "Shared player data" below.
