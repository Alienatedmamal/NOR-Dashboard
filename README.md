...
- **Players** - online players (name, SteamID, IP, ping, session/total time, last connected, Rust hours) with one-click kick, ban/unban (reason required and logged), and Look up; plus recently-seen offline players, currently-banned players, and a per-player notes log (ban reasons are added automatically). Notes and stats (total time, last connected) are shared with any other admin running their own copy of this dashboard, if Settings > Plugin Deploy is configured - see "Shared player data" below.
